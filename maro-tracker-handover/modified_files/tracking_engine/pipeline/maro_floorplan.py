...
class MaroFloorplanAssets:
    ...
    def mm_to_plan_px(self, x_mm: float, y_mm: float) -> tuple[float, float]:
        # Maro's render_floorplan_png.py applies ax.invert_yaxis(), i.e. the PNG
        # already has min_y at the top, max_y at the bottom. No extra flip here.
        b = self.bounds_mm
    ...
    def overlay_for_ui(self) -> dict[str, Any]:
        ...
        for s in self.spots:
            pos = s.get("position")
            # Maro liefert position als Liste [x, y]; falls dict mit x/y kommt: auch supporten
